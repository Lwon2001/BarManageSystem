...
int single;
int rest;
int RootPasswordFlag = 1;// 1表示Root管理页面需要输入密码进入
...
VIP *VIPReadFile();  //从文件中读取会员数据
void VIPInputFile(VIP *VIPlisthead);  //将会员数据写入文件
void ClearArr_int(int *a, int size, int value);  //int型数组的初始化
void ClearArr_char(char *a, int size, char value);  //char型数组的初始化
void VIP__init__(VIP *node);  //VIP结构体的初始化函数
struct tm *GetNowTime();  //获取当前时间
int Register_tel(VIP *node,VIP *head);  //用电话号码进行注册会员或员工
int Register_recharge(VIP *node);  //会员充值
int VIPCheck_money(char *money);  //检测输入金额的合法性
int C_INTtransf(char *money);  //char类型数字->int类型
void ConsumeRecord(VIP *node, Complete_Form *head);  //展示会员消费记录
void Add_VIPOrders(VIP *node, Complete_Form *cf);  //添加会员订单
int inputYorN();  //Y返回1N返回0ESC返回-1
void V_order_char(int version);  //对VIP的ID（时间）进行排序 1正序 0倒序
void V_order_float(int version);  //对VIP的余额进行排序 1正序 0倒序
void F_order_char(int version);  //对staff的ID（时间）进行排序 1正序 0倒序
void F_order_float(int version);  //对staff的工资进行排序 1正序 0倒序
...
VIP *Find_Vip(VIP *head, int index);  //在会员链表中寻找id为index的会员结点，返回指向该结点的指针
STAFF *STAFFReadFile();  //从文件中读取员工数据
void STAFF__init__(STAFF *node);  //员工结点初始化，返回无哨兵结点
void STAFFInputFile(STAFF *head);  //将员工数据写入文件
...
VIP *VIPRegister(VIP *VIPlisthead);  //注册会员
VIP *VIPlogin(VIP *VIPheadlist);  //会员登陆
void STAFFoperationUSER(STAFF *node);  //user会员操作
void VIPoperationUSER(VIP *node);  //user会员操作
void ShowMonthSales(int a);  //展示各类饮品的月销量
void ShowSeasonSales(int a);  //展示各类饮品的季度小料
void ShowYearSales(int a);  //展示各类饮品的年度小料
void Check_Month_Sales();  //检查月销量
void Check_Season_Sales();  //检查季度销量
void Check_Year_Sales();  //检查年度销量
void ShowSale();  //查看各饮品销量
void AddGoods();  //添加商品
void DeleteGoods();  //删除商品
void ManageGoods();  //商品管理
void ModifyPrice(); //修改商品价格
void ChangeGoods();  //增删商品种类
int Show_Goods(Beverage *h);   //展示饮品类型,返回当前饮品个数
float CheckFloat(char a[],int len);  //检查输入是否为正浮点数
int EnsureDelete(Beverage* p);  //二次确认删除该商品,确认返回1，取消删除返回-1
int RootLogin();  //ROOT管理页面的登陆页面
...
int Check_Number(char ch[],int maxnum,int maxlen);  //检查数字是否合规(合规且不超过maxnm返回对应的数字值，合规但超过max返回-1，不合规返回-2)
void color(int x);  //设置字体颜色
void setColor(unsigned short ForeColor,unsigned short BackGroundColor);  //设置输出窗口背景颜色与字体颜色
...
{
    int num=0;
    int x1=22,x2=34,x3=46,y=13;  //光标所在的位置坐标
    if(h->next==NULL)
    {
        toxy(X_COORD+x1,y);
...
void Choose_Payway(Complete_Form* cf)  //选择支付方式
{
    system(Normal_Color);;  //设置背景颜色与字体颜色
    system("cls");  //清屏
    HideCursor(0);  //隐藏光标
...
void Add_Inventory()  //添加库存
{
    ...
    else if(x==-2)
    {
        HideCursor(0);
        toxy(X_COORD+21,35);
        printf("输入内容不合规,请1s后重新输入原材料对应的ID");
        Sleep(1000);
        Add_Inventory();  //递归直至输入合规
        return;
    }
...
void Inventory_GetAdded(Material *m)  //用户选择添加的量，返回用户的选择(1---1kg，2---加至半满，3---加满)
{
    ...
    switch(t)
    {
        case '1':
            ...
            {
                toxy(X_COORD+30,18);
                printf("%s库存超过一半，添加失败",m->name);
            }
            toxy(X_COORD+30,19);
            printf("2秒后自动返回库存管理页面");
            Sleep(2000);
            Inventory_Manage();
            return;
        case '2':
...
void ChangeShowWay(Consumption *head,Consumption *tail,int status)  //根据status的值来选择是按消耗量从小到大，还是按消耗量从大到小来显示原材料的总消耗量(status=1时候从小到大,0时从大到小)
{
    ...
    Consumption *con;
    int i = 1; //计数器
    int x1=24,x2=36,x3=48,y=11;  //光标所在的位置坐标
    toxy(X_COORD+28,8);
    printf("  时尚泡吧库存管理系统");
    toxy(X_COORD+22,9);
    printf("-------------------------------------");
...
void ShowYearSales(int a)
{
    ...
    printf("-------------------------------------");
    toxy(X_COORD+27,11);
    printf("上一年各种饮品的销量如下：\n");//这里要根据a判断是本年度还是上一年度，用switch case解决
    toxy(X_COORD+27,12);
    printf("---------------------------\n");
...
void Check_Season_Sales()
{
    ...
    while(1)
    {
        ...
        printf("-------------------------------------");
        toxy(X_COORD+27,11);
        printf("1.春季\t2.夏季\t3.秋季\t4.冬季");
        toxy(X_COORD+27,12);
        printf("请输入2020年你要查看的季度（0返回）：");
        //system("pause");
        fflush(stdin);
        toxy(X_COORD+64,12);
        scanf("%s",a);
...
void Check_Year_Sales()
{
    ...
    while(1)
    {
        ...
        if(b==-1||b==-2)
        {
            ...
            printf("-------------------------------------");
            toxy(X_COORD+27,11);
            printf("请输入数字0-2！\n");//发现一个bug，输入错误之后要按两下以上0才可以退出。(已完成)
            Sleep(1000);
            system("CLS");
...
float CheckFloat(char a[],int len)  //检查输入是否为正浮点数，且只能精确到小数点后一位
{
    int i,j,k;
    int dot_num = 0;  //小数点的个数
    float integer=0;  //整数部分的值
    float dot=0;  //小数部分的值
    int dot_place;  //小数点的位置
    for(i=0;i<len;i++)
    {
        if((a[i]<'0'||a[i]>'9')&&a[i]!='.')
        {
...
void EnsureModify(Beverage* p,float c)
{
    ...
    printf("-------------------------------------");
    toxy(X_COORD+26,10);
    printf("%s的价格即将从%.1f更改为%.1f:",p->name,p->price,c);
    toxy(X_COORD+36,13);
    printf("1.确认");
    toxy(X_COORD+36,15);
    printf("2.取消");
    ...
    switch(a)
    {
        ...
        case '2':
            toxy(X_COORD+26,17);
            printf("取消成功!!!2s后将自动返回修改商品价格页面");
            Sleep(2000);
            ModifyPrice();
            return;
        default:
            toxy(X_COORD+24,17);
            printf("输入不合规，请根据标号进行选择");
            EnsureModify(p,c);
            return;
    }
}
...
void ModifyPrice()//调整商品价格函数设计思路：1输入商品名字2查找2.1找到就显示价格并且提示改为多少钱2.2没找到就提示没找到是否继续找3继续：调用本函数3.1不继续：返回商品管理菜单
{
    ...
    if(re>0)
    {
        while(1)
        {
            ...
            printf("-------------------------------------");
            toxy(X_COORD+26,13);
            printf("请输入小于等于100的非负浮点数");
            toxy(X_COORD+26,14);
            printf("输入的浮点数应精确到小数点后一位");
            toxy(X_COORD+26,11);
            printf("请输入%s修改后的价格:",q->name);
            fflush(stdin);
            scanf("%s",b);
            fflush(stdin);
...
void AddGoods()  //bug这里添加商品界面在shell里显示有点奇怪，这应该是AddGoods函数上一级的失误
{
    char a[21];//用来储存输入的名字
    char c;//c用来选择的
...
void DeleteGoods()  //删除商品页面.bug添加饮料之后，删除商品界面并没有出现。
{
    system(Normal_Color);;  //设置背景颜色与字体颜色
    system("cls");  //清屏
    HideCursor(0);  //隐藏光标
    char a[20];  //存储用户输入
    int beverage_num;  //商品数量
    int re;
    toxy(X_COORD+28,8);
    printf("   时尚泡吧商品管理系统");
    toxy(X_COORD+22,9);
    ...
    if(re>0)
    {
        Beverage *q,*q0;  //找到饮品中id为c的饮品种类，用q指向,q0指向其前驱
        q = Find_Beverage(re,head_beverage);
        if(EnsureDelete(q)>0)  //确认删除
        {
            system("cls");
            if(re==1)
            {
            ...
            q0->next = q->next;
            while(q0->next!=NULL)  //修改删除商品后各商品的id
            {
                q0->next->id-=1;
                q0 = q0->next;
            }
            ...
            printf("-------------------------------------");
            toxy(X_COORD+26,12);
            printf("商品:%s已删除!!!",q->name);
            toxy(X_COORD+26,14);
            printf("两秒后返回选择界面");
            free(q);  //释放被删除商品对应结点的空间
            Sleep(2000);
            DeleteGoods();
            return;
        }
...
void ManageGoods()//bug商品管理这里返回上一级全部都有问题。
{
    char a;
    system(Normal_Color);;  //设置背景颜色与字体颜色
    system("cls");  //清屏
