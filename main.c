...
void Order()
{
    ...
    if(node_VIP!=NULL)  // 如果是会员登录界面，node会赋值，否则node赋值NULL
    {               //如果是VIP给is_member赋值
        CF->is_member = 1;
    }
...
void Choose_Ingredients(Complete_Form* cf,int i)
{
    ...
    Material* ci_return = Consume_Inventory(cf);  //根据该订单信息对库存进行“消耗”
...
int Register_password(VIP *node){
    ...
    while(1)
    {
        if(!norepeat){//确认这个conyinue不是因为输入错误造成的，而是为了输入repeat
            for (int i = 0; i < 100;i++)
                password[i] = 0;
        }
        else{
        ...
        if(norepeat){
            ...
            if(norepeat){
                toxy(48 + lenpassword + 1, 11);
            }
            else{
                toxy(48 + lenpassword + 1, 10);
            }
            color(16);
        }
        else
...
void VIPUSERshow(VIP *node){
    system("cls");
    printf("卡号：%s\n姓名：%s\n电话：%s\n卡上余额：%.2f\n卡状态：", node->id.id,node->name ,node->tel, node->restmoney);
    if(node->loss){
        color(4);
        printf("挂失\n");
        color(17);
    ...
    if(node->loss){
        printf("请输入操作：1.充值 2.解挂 3.查看消费记录 4.修改密码 5.修改电话 *返回 ESC.退出登录\n");
    }
    else{
        printf("请输入操作：1.充值 2.挂失 3.查看消费记录 4.修改密码 5.修改电话 6.会员点餐 *返回 ESC.退出登录\n");
    }
}
// 操作反馈成功界面
...
void VIPadminoperation(){
    ...
    while(1){
        ...
        printf("\n\n\n");
        printf("1.增加会员 2.注销会员 3.修改/查看会员详细信息 4.按时间排序 5.按余额排序 ESC.返回主菜单 *返回\n");
        while(1){//输入管理
            int choose = 0;
            fflush(stdin);
            choose = getch();
...
void VIPdetele(VIP *head){
    ...
    while(1){//输入编号
        ...
        printf("请输入要注销会员的编号：（*返回 ESC返回主菜单）");
        while(1){
            fflush(stdin);
            temp = getch();
            fflush(stdin);
...
void ModandShowDetail(VIP *head){
    ...
    while(1){//输入编号
        ...
        printf("请输入要修改查看会员的编号：（*返回 ESC返回主菜单）");
        while(1){
            fflush(stdin);
            temp = getch();
            fflush(stdin);
...
void STAFFUSERshow(VIP *node){
    if(node==NULL)
        return;
    system("cls");
    printf("工号：%s\n姓名：%s\n电话：%s\n工资：%.2f\n密码：******\n职位：", node->id.id,node->name ,node->tel, node->restmoney);
    switch (node->position)
    {
        case 1:
            printf("服务员\n");
    ...
    if(node->position==1){
        printf("\n请输入操作：1. 修改登录密码 2.修改手机号 3. 查看业绩 4.点餐 5.会员注册 6.会员登录 *返回 ESC.退出登录\n");
    }
    else{
            ...
            default:
                ...
                if(operation=='4'&&node->position==1){
                    ...
                    while(1){
                        system("cls");
                        node_STAFF = node;
                        printf("点餐顾客是会员吗？（Y/N）*返回 ESC返回主菜单");
...
void Default(){
    while(1){
        ...
        if(ExplicitFlag){
            system("cls");
            printf("你好呀大傻逼，欢迎来到时尚泡吧\n");
            printf("1.管理员滚这里 2.会员登录 3.会员注册 4.非会员点餐 5.爹 ESC.退出");
        }
        char choose = 0;
        VIP *node_main;
        fflush(stdin);
...
void ROOT(){
    ...
    while(1){
        ...
        char choose = 0;
        printf("爹 你来啦！");
        printf("\n1.员工管理 2.库存管理 3.商品管理 4.业务管理 5.会员管理");
        choose = getch();
        switch(choose){
            case '1':
                system("cls");
...
int RootLogin(){
    ...
    while(1){
        ...
        int over = 0;
        char word=0;
        char temp[100] = "请输入密码我的大爹(ESC返回主菜单 *返回)：";

        printf("请输入密码我的大爹(ESC返回主菜单 *返回)：");
        if(error){
            color(4);
            printf("\n密码错误!!");
            color(17);
